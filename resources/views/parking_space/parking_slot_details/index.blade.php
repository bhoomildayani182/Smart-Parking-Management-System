...
@slot('title') Parking Slot Details  @endslot
@endcomponent
                    ...
                    <table class="table align-middle table-nowrap" id="customerTable">
                        <thead class="table-light text-muted">
                            <tr>
                                ...
                                <th class="sort" scope="col">Space Name</th>
                                <th class="sort" scope="col">Slot</th>
                                <th class="sort" scope="col">Booked</th>
                            </tr>
                        </thead>
                        <!--end thead-->
                        <tbody class="list form-check-all">
                            @foreach ($parkingslotdetails as $parkingslotdetail)
                                <tr>
                                    <td scope="col">{{ request('page') !== null ? (request('page') - 1) * 10 + $loop->iteration  :  $loop->iteration}}</td>
                                    <td>{{ $parkingslotdetail->space_name }}</td>
                                    <td>{{ $parkingslotdetail->slot->name }}</td>
                                    <td><span class='badge text-uppercase {{ ($parkingslotdetail->is_booked == true) ? 'badge-soft-danger' : 'badge-soft-success' }}'>{{ ($parkingslotdetail->is_booked == true) ? 'Booked' : 'Avilable' }}</span></td>
                                </tr>
                            @endforeach
