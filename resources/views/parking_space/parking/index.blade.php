    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex align-items-center border-0">
                <h5 class="card-title mb-0 flex-grow-1">Parking List</h5>
                <div class="flex-shrink-0">
                    <a href="{{ route('parking.create')}}" class="btn btn-success"><i class="ri-add-line align-bottom me-1"></i> Add Parking</a>
                </div>
            </div>
            ...
            <div class="card-body">
                    ...
                    <table class="table align-middle table-nowrap" id="customerTable">
                        <thead class="table-light text-muted">
                            <tr>
                                ...
                                <th class="sort" scope="col">Parking Name</th>
                                <th class="sort" scope="col">State Name</th>
                                <th class="sort" scope="col">City Name</th>
                                <th class="sort" scope="col">Manager Name</th>
                        ...
                        <tbody class="list form-check-all">
                            @foreach ($parkings as $parking)
                                <tr>
                                    ...
                                    <td>{{ $parking->state }}</td>
                                    <td>{{ $parking->city }}</td>
                                    <td>{{ $parking->manager_name }}</td>
                                    <td class="status"><span class='badge text-uppercase {{ ($parking->status == 'ACTIVE') ? 'badge-soft-success' : 'badge-soft-danger' }}'>{{ $parking->status }}</span></td>
                                    <td>
                                        <div class="hstack gap-2">
                                            <a class="btn btn-sm btn-soft-danger remove-list" href="{{ route('parking.delete', ['parking' => $parking->id]) }}"><i class="ri-delete-bin-5-fill align-bottom"></i></a>
                                            <a class="btn btn-sm btn-soft-info edit-list" href="{{ route('parking.update', ['parking' => $parking->id]) }}"><i class="ri-pencil-fill align-bottom"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                ...
                {!! $parkings->withQueryString()->links('admin.paginate') !!}
